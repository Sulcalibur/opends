...
export function handleApiError(event: H3Event, error: unknown) {
  ...
  if (error instanceof Error) {
    ...
    if (error.message.includes("not found")) {
      ...
      return createErrorResponse(ErrorCodes.NOT_FOUND, error.message);
    }
    ...
    if (
      ...
      return createErrorResponse(ErrorCodes.UNAUTHORIZED, error.message);
    }
    ...
    if (
      ...
      return createErrorResponse(ErrorCodes.FORBIDDEN, error.message);
    }
    ...
    if (
      ...
      return createErrorResponse(ErrorCodes.DUPLICATE_ENTRY, error.message);
    }
 ...
 */
export function asyncHandler(handler: (event: H3Event) => Promise<unknown>) {
