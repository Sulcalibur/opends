      ...
      <div class="max-w-6xl mx-auto">
        ...
        <div class="mb-8">
          ...
          <p class="text-gray-600">
            Core design decisions including colors, typography, spacing, and
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          ...
          <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <p class="text-gray-500 text-sm">
              Consistent spacing scales and measurements.
            </p>
          </div>
        </div>
        ...
        <div v-else class="space-y-8">
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            ...
            <div v-else class="text-center py-8 text-gray-500">
              ...
              <NuxtLink
                ...
                >Add some in the admin panel</NuxtLink
              >.
            </div>
          </div>
          ...
          <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-8">
            ...
            <div v-if="spacingTokens.length > 0" class="space-y-4">
              ...
              >
                <code
                  class="text-sm bg-gray-100 px-2 py-1 rounded min-w-[100px]"
                  >{{ token.key }}</code
                >
                <div
...
const colorTokens = computed(() =>
  tokens.value.filter(
    (token) => token.category === "color" || token.key.includes("color"),
  ),
);
...
const typographyTokens = computed(() =>
  tokens.value.filter(
    (token) =>
      ...
      ["font", "text", "line"].some((k) => token.key.includes(k)),
  ),
);

const spacingTokens = computed(() =>
  tokens.value.filter(
    (token) => token.category === "spacing" || token.key.includes("space"),
  ),
);
  ...
  try {
    ...
    tokens.value = [
      ...
      // Spacing
      {
        ...
        value: "0.25rem",
        category: "spacing",
        description: "Extra small space (4px)",
      },
      {
        key: "--space-2",
        value: "0.5rem",
        category: "spacing",
        description: "Small space (8px)",
      },
      {
        key: "--space-3",
        value: "0.75rem",
        category: "spacing",
        description: "Medium small space (12px)",
      },
      {
        key: "--space-4",
        value: "1rem",
        category: "spacing",
        description: "Medium space (16px)",
      },
      {
        key: "--space-6",
        value: "1.5rem",
        category: "spacing",
        description: "Large space (24px)",
      },
      {
        key: "--space-8",
        value: "2rem",
        category: "spacing",
        description: "Extra large space (32px)",
      },
      {
        key: "--space-12",
        value: "3rem",
        category: "spacing",
        description: "2XL space (48px)",
      },
      {
        key: "--space-16",
        value: "4rem",
        category: "spacing",
        description: "3XL space (64px)",
      },
