<template>
  <div class="min-h-screen bg-gray-50 flex">
      ...
      <div class="max-w-7xl mx-auto">
        ...
        <header class="mb-8 flex justify-between items-end">
          ...
          <div class="flex gap-3">
            <UButton variant="outline" color="neutral" size="sm" icon="i-lucide-external-link">View Styleguide</UButton>
            <UButton size="sm" icon="i-lucide-refresh-cw" @click="showImportModal = true">Sync from Penpot</UButton>
          </div>
        </header>
        ...
        <div class="grid grid-cols-12 gap-8">
          ...
          <div class="col-span-12 lg:col-span-8">
            <h2 class="text-lg font-semibold text-gray-900 mb-4 flex items-center gap-2">
              <UIcon name="i-lucide-book" class="text-gray-400" /> Your Styleguides
            </h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                ...
                <div class="h-32 bg-gradient-to-r from-indigo-500 to-purple-500 relative">
                  <div class="absolute bottom-4 left-4">
                    <div class="w-10 h-10 bg-white rounded-md flex items-center justify-center shadow-sm">
                      <UIcon name="i-lucide-box" class="text-indigo-600 text-xl" />
                    </div>
                  </div>
                </div>
                <div class="p-5">
              ...
              <div class="bg-gray-50 rounded-lg border-2 border-dashed border-gray-300 hover:border-indigo-400 hover:bg-gray-100 transition-all cursor-pointer flex flex-col items-center justify-center p-8 text-center h-full min-h-[240px] group">
                <div class="w-12 h-12 rounded-full bg-white border border-gray-200 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform">
                  <UIcon name="i-lucide-plus" class="text-gray-400 group-hover:text-indigo-600" />
                </div>
            ...
            <div class="bg-white rounded-lg border border-gray-200 shadow-sm p-6 mb-8">
              ...
              <div class="space-y-4">
                <div class="flex items-center justify-between p-4 bg-blue-50 rounded-md border border-blue-100">
                  ...
                  <UButton size="sm" variant="outline" color="neutral" class="bg-white" @click="showImportModal = true">Sync Now</UButton>
                </div>
    ...
    <!-- Import Modal -->
    <UModal v-model="showImportModal" title="Sync from Penpot">
      <div class="space-y-6 pt-2">
        <div class="bg-blue-50 border border-blue-100 rounded-md p-4 flex gap-3">
          <UIcon name="i-lucide-info" class="text-blue-600 mt-0.5" />
          <p class="text-sm text-blue-800">
            Upload your Penpot export file (<code>.json</code>) to sync tokens and structure.
          </p>
        </div>
        ...
        >
          <UIcon name="i-lucide-cloud-upload" class="text-3xl text-gray-400 mb-3" />
      ...
      <template #footer>
        <div class="flex justify-between w-full pt-4">
          ...
          <div class="flex gap-2">
            ...
            <UButton @click="triggerFileInput">Import</UButton>
          </div>
        </div>
      </template>
    </UModal>
    ...
    <UModal v-model="showClearConfirm" title="Clear Data">
      <p class="text-gray-600">Are you sure? This will remove all local data.</p>
      <template #footer>
        <div class="flex gap-2 justify-end">
          <UButton variant="ghost" color="neutral" @click="showClearConfirm = false">Cancel</UButton>
          <UButton color="error" @click="clearAllData">Yes, Clear</UButton>
        </div>
      </template>
    </UModal>
  </div>
...
function loadStats() {
  ...
  if (stats.components > 0) {
    recentActivity.value = [
      { id: 1, description: 'Added Button component', time: '2 mins ago' },
      { id: 2, description: 'Updated color tokens', time: '1 hour ago' }
    ]
  } else {
    recentActivity.value = []
  }
